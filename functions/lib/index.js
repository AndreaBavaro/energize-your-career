...
exports.cleanupInactiveSubscribers = exports.manualSendBlogNotification = exports.sendNewBlogPostNotification = void 0;
 ...
 */
exports.sendNewBlogPostNotification = functions.firestore
 ...
 */
exports.manualSendBlogNotification = functions.https.onCall(async (data, context) => {
    // Check if the request is made by an admin (you would implement proper auth checks)
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to trigger notifications');
    }
    try {
        const { postId } = data;
        if (!postId) {
        ...
        if (!postDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Blog post not found');
        }
 ...
 * Runs once a month
 */
exports.cleanupInactiveSubscribers = functions.pubsub
    ...
    try {
        ...
        const updatePromises = inactiveSnapshot.docs.map(doc => doc.ref.update({ isActive: false }));
